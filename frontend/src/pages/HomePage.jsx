...
const HomePage = () => {
  ...
  const renderRecommendedUsers = () => {
    if (loadingUsers) return renderLoadingState()
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 bg-base-200 p-4 rounded-lg">
        {recommendedUsers.map((user) => {
          const hasRequestBeenSent = outgoingRequestsIds.has(user._id)
          const learningLanguages = getLearningLanguages(user.learningLanguage)
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {learningLanguages.map((lang) => (
                      <span key={lang} className="badge badge-accent text-xs">
                        {getLanguageFlag(lang)} Learning: {capitalize(lang)}
                      </span>
                    ))}
                  </div>
                </div>
  ...
  const renderFriendsSection = () => {
    if (loadingFriends) return renderLoadingState()
